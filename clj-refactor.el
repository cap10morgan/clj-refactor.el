...
This makes `cljr-replace-refer-all' as it can be."
  :group 'cljr
  :type 'boolean)
  ...
  (when (search-forward ":require" nil t)
    (let (nses
      ...
      (if (not (save-excursion (re-search-forward ":refer\s:all" require-end t)))
        ...
        (while (save-excursion (re-search-forward ":refer\s:all" require-end t))
          (re-search-forward ":refer\s:all" require-end t)
          (paredit-backward-up)
          (let ((prefix (save-excursion
                          (when (progn
                                  (paredit-backward-up)
                                  (not (looking-at "(:require")))
                            (paredit-forward-down)
                            (buffer-substring (point) (cljr--point-after 'paredit-forward)))))
                (ns (progn
                      (paredit-forward-down)
...
(defun cljr--replace-refer-all-with-referred-list (ns publics-occurrences)
  (when (re-search-forward
...
;;;###autoload
(defun cljr-replace-refer-all (with-referred-list)
  "Replaces :refer :all style require with alias :as style require.
    ...
    in the namespace.
    With prefix WITH-REFERRED-LIST replaces refer all with the list of
    public symbols used in the namespace.
  ...
  (let ((refer-all-nses (cljr--extract-refer-all-namespaces))
    ...
    (dolist (refer-all-ns refer-all-nses)
      (cljr--goto-ns)
      (let* ((alias (when (not with-referred-list)
             ...
             (occurrences (->>  (cljr--call-middleware-sync request "used-publics")
                                (edn-read))))
        (if with-referred-list
            (cljr--replace-refer-all-with-referred-list refer-all-ns occurrences)
